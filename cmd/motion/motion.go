...
// Deps holds injectable dependencies for the motion commands.
type Deps struct {
...
// Command returns the top-level "motion" cobra command group.
...
// CommandWithDeps constructs the "motion" cobra command group with injected dependencies.
func CommandWithDeps(deps Deps) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "motion",
		Short: "Motion control operations on a named printer",
	}
	cmd.AddCommand(homeCommandWithDeps(deps))
	cmd.AddCommand(jogCommandWithDeps(deps))
	return cmd
}
