...
`--insecure` may be passed to `printer add`, `printer status`, `camera stream`, `files roots`, `files list`, `files download`, `files upload`, or `printer tls refresh`.
...
When `--insecure` is passed to `printer status`, `camera stream`, `files roots`, `files list`, `files download`, or `files upload`:
