// Package h264decode provides a cgo-based H.264 frame decoder using libavcodec.
// It decodes H.264 access units (as NALU slices) into Go image.RGBA frames.
package h264decode
...
// Decoder decodes H.264 access units into RGBA images using libavcodec.
type Decoder struct {
...
// New creates a new H.264 frame decoder.
...
// Close releases all libavcodec resources.
...
func (d *Decoder) reinitRGBAFrame() error {
	if d.swsCtx != nil {
		C.sws_freeContext(d.swsCtx)
	}
	if d.rgbaFrame != nil {
...
// ContainsSlice reports whether au contains an actual picture slice
// (IDR or non-IDR), as opposed to only parameter sets or SEI data.
func ContainsSlice(au [][]byte) bool {
	for _, nalu := range au {
		if len(nalu) == 0 {
			continue
		}
...
// Decode decodes a single H.264 access unit (slice of NALUs) into an RGBA image.
// Returns nil image when the decoder needs more data (e.g. parameter sets only).
func (d *Decoder) Decode(au [][]byte) (*image.RGBA, error) {
